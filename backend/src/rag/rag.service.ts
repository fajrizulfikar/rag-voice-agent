...
export class RagService {
  ...
  async processTextQuery(textQueryDto: TextQueryDto): Promise<QueryResponse> {
    ...
    try {
      ...
      const similarDocuments = await this.vectorService.searchSimilarDocuments(queryEmbedding);
  ...
  async processVoiceQuery(voiceQueryDto: VoiceQueryDto): Promise<QueryResponse> {
  ...
  private async generateAnswer(query: string, documents: any[]): Promise<string> {
  ...
  private async updateQueryLog(id: string, updates: Partial<QueryLog>): Promise<void> {
